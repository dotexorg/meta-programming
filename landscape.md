...
AGENTS.md got its own counter-intuitive result (arxiv 2602.11988): auto-generated context files produce no significant change in success rates while adding roughly 20% cost, and file length has no measurable effect on outcomes. The often-repeated "500-line cliff" is not in the paper; reading it firsthand disconfirms any length threshold. What holds independently is adherence: practitioners report context-file rules are followed only about 70% of the time, which is the real reason to keep must-hold constraints in hooks rather than prose.
...
A fourth angle on the same problem reached production in May 2026. The edit-reliability fix doesn't always need a better text-merge layer — it can also remove the failure at the discovery step, before the edit happens. One open-source MCP server (agent-lsp) bridges Language Server Protocol into the agent's tool surface and ships speculative execution as a built-in: a hypothetical edit applies in memory, the language server returns its diagnostic delta, and only an empty-or-expected delta commits to disk. Across five real codebases spanning 15K to 319K lines of Go, Python, and TypeScript, 92–99% of grep matches on symbol references turn out to be false positives at scale — HashiCorp Consul `Close` returns 1156 grep hits and twelve real references. Structured navigation runs five to thirty-four times more token-efficient than grep-and-read, scaling with codebase size. Bridge quality inherits whatever the underlying language server can see, which makes the approach reliable for typed languages and weaker for dynamic ones, and the daemon takes minutes of cold indexing on Python and TypeScript projects before the first useful query. 🟡

## Recovery loops crystallize

The Ralph-loop pattern reached its fourth independent shipped implementation by May 2026. 🟡 The recipe is consistent across all four: fresh context per iteration so failures don't bleed across attempts, external memory in files plus Git so state survives the reset, one item per loop so each iteration fits the context window, and a machine-verifiable completion criterion so success isn't a self-report. The sightings differ in how much architecture is exposed: Geoffrey Huntley named the pattern in his original blog and on `ai-assisted-software-development.com`, LoopTroop wrapped it in a three-stage "councils plan, Ralph loops recover, OpenCode worktrees ship" framing in April, `ralph-claude-code` extracted the recovery layer alone with cost caps and an MCP audit mode, and one Rust implementation (lopi) shows the loop in full source: Plan → Implement → Test → Score → Fix-in-place → Retry with `git reset --hard` per failed attempt, plus a post-mortem step on terminal failure that distills the failure into a single imperative constraint stored for retrieval on the next similar run.

The pattern that compounds is the quality gate paired with extraction. An unfiltered extractor degrades future retrieval because every weak lesson it writes lowers the signal-to-noise ratio of the store it's supposed to improve. The same orchestrator addresses this by skipping writes when a run scores below `LESSON_QUALITY_GATE = 0.6` — the run isn't informative enough to generalise from, so persisting it would compound noise alongside signal. Extraction without a quality gate is the common shape across published self-improvement loops; the gate is what keeps the loop from poisoning itself over time. 🟡
