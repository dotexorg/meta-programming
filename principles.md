...
There's a caveat to this principle that's worth stating directly. A large April 2026 trajectory study (9,374 agent runs across 19 agents) found that the successful behavioral pattern — gather context before editing, invest in validation — is agent-determined, not task-adaptive. Framework prompts still influence tactics, but the effect narrows with each generation of base model. The practical consequence: our pipeline's alpha over a raw Opus 4.7 prompt will be smaller than its alpha was over Sonnet 4.6. That doesn't invalidate the principle. It shifts what to measure. Track the pipeline-vs-raw delta over time. When it narrows to single digits, the remaining value lives in the parts models can't internalise: boundaries the model won't invent, reviewers with fresh context, artefacts that persist across sessions.
...
A controlled academic experiment (Spec Gap, ICN2 Barcelona, March 2026) put exact numbers on the same pattern. 51 class-generation tasks, four spec detail levels from full docstrings to bare signatures. Single-agent pass rate moved from 89% to 56% as detail was removed. Multi-agent decomposition moved from 58% to 25% on the same tasks. The counterintuitive part was the recovery experiment: adding an AST-level conflict detector at 97% precision shifted scores by zero, while restoring the full spec at merge time recovered the 89% ceiling. **Specifications are simultaneously the cause of coordination failure and the sufficient instrument of recovery.** Invest in the spec; skip the reconciliation tooling.
...
ETH Zurich measured the same pattern: auto-generated context files produced no significant success gain across 138 tasks while adding cost. Self-written context spends effort without buying direction, and that penalty bites hardest when the task requires finding non-obvious solutions.

The cost of WHERE-style hints has cross-codebase measurement now. Across five real codebases spanning 15K to 319K lines of Go, Python, and TypeScript, 92–99% of grep matches on symbol references turn out to be false positives — HashiCorp Consul `Close` returns 1156 grep hits and twelve real references, FastAPI `validate` returns 64 hits and two, Hono `Close` returns 15 hits and one (agent-lsp benchmark, May 2026). Telling the agent "start with grep X" doesn't just narrow the search space; it routes ninety-plus percent of the resulting attention onto false positives, which the agent then has to read and discard before reaching the real reference. Structured navigation primitives — LSP `find_references`, symbol-graph queries — escape the noise rate by construction rather than by reading more files. The principle had a single in-house measurement before; now it has cross-codebase measurement of the specific mechanism it warns against. 🟡
...
Extraction asks: what specifically happened in this session? Factual summarization. A cheap model handles it well. Promotion asks: what generalizable rule does this represent? Synthesis from specific cases to general patterns correlates directly with model capability. Run promotion with a weak model and the KB fills with case-specific noise masquerading as universal rules.
...
**Form selection for Layer 2.** The principles above express WHAT and WHY in natural language. The open question isn't what new language to invent — the available forms (NL, AGENTS.md, SKILL.md, DO/DO NOT, rules, hooks) already cover the space. The question is which form fits which task. Rough heuristic from our own use: one-off exploration stays in NL; cross-session conventions in AGENTS.md; reusable procedures in SKILL.md; boundary-sensitive tasks in DO/DO NOT/GLOSSARY; non-negotiable safety gates in hooks. But the decision tree isn't systematised. Different practitioners pick different forms for the same task type and get different outcomes. Measuring which form wins for which task class is the unresolved part — and it has to happen before external tooling (ContextCov, IntentLang) pays off. **Update (May 2026):** the form taxonomy gained a named seventh entry — single-imperative (one line, `must` / `do not` / `always` / `never`, ≤200 chars) — shipping in lopi for cheap per-session post-mortem capture. The size axis now has both ends populated with reference implementations: single-imperative at the small end, multi-section AGENTS.md at the large end. The decision tree question narrows: not "what forms exist" but "which size threshold favors which form."

**Pipeline alpha under stronger models.** The Behavioral Drivers finding (agent-determined, not task-adaptive) implies that framework effects diminish with each model generation. If the trend is monotonic, most pipeline infrastructure becomes legacy overhead within two years. If it's non-monotonic (structure still buys something at long horizons or specific task types), the investment compounds. We have no longitudinal data across model generations yet — the baseline year (2026) is what we're living through. Experiment design: rerun our Experiment 1 variants across model versions as they ship, track whether pipeline-vs-raw delta narrows.
