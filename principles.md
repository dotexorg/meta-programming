...
The May 2026 agent-lsp benchmark put a number on the cost of WHERE-style hints at scale. Across five real codebases (Go, Python, TypeScript, 15K to 319K LOC), 92–99% of grep matches on symbol references were false positives — HashiCorp Consul `Close` returns 1156 grep hits and twelve real references; FastAPI `validate` returns 64 hits and two; Hono `Close` returns 15 hits and one. Telling the agent "start with grep X" doesn't just narrow the search space; it routes 90%+ of the resulting attention onto false positives. Structured navigation primitives (LSP `find_references`, symbol graph queries) escape the noise rate by construction. The principle had measurement before; now it has cross-codebase measurement of the specific mechanism it warns about. 🟡
