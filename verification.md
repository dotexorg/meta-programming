...
May 2026 added two production-grade primitives at this layer worth naming. agent-lsp ships **phase enforcement** as a runtime gate — a phase is declared (`scout`, `plan`, `implement`, `review`), the binary rejects tool calls that don't fit the active phase, and the rejection is structural rather than advisory. This is the shipped form of the hooks-as-gates pattern Claude Code documented as concept-only in Chapter 12: a non-negotiable boundary enforced by exit code, not by prompt hygiene. Same binary ships **speculative execution** as an agent-native primitive: apply a hypothetical edit in memory, ask the language server for the diagnostic delta, commit only if the delta is empty or expected. The edit never touches disk on a failed speculation. This is preflight pulled one step earlier than `tsc --noEmit` — the type-check happens before the file is written, not after, and the failed attempt costs zero retry tokens. Neither primitive is a model-quality fix; both are gates that keep the existing model from corrupting state through plausible-but-wrong edits. 🟡
