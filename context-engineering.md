...
Chroma tested 18 frontier models in 2025 and every one degraded with input length. Accuracy dropped 20-50% as input grew from 10K to 100K tokens; mid-window positions lost over 30% accuracy across the board. A separate paper that year (Paulsen's work on the Maximum Effective Context Window) showed the collapse is task-dependent: simple retrieval tolerates a few thousand tokens, but complex operations like sorting or summarization break down at 400-1200. We've seen the pattern ourselves: Opus 4.6 started ignoring instructions at roughly 40% context fill. Not with an error. Just silently skipping rules set an hour earlier. A task that passed in a fresh session would fail mid-context with no signal. That silence is the dangerous part.
...
Our knowledge base holds ~1,030 bullets, roughly 180K tokens of raw archive. Loading it all into every session would consume the entire window before any work started, and most would land in the lost-in-middle zone anyway. The architecture splits the material across four roles. The **raw archive** (`meta.json`) is searchable via a tool call, never loaded whole. A **curated core** (`tier1-core.md`, ~12KB) covers thesis, principles, our experiments. It's the always-load layer, deliberately kept small to stay within the model's practical rule budget. **Topic tiers** (`tier2-*.md`, 4–18KB each) load on-demand when the task touches that area. A **synthesis doc** sits between the raw KB and the tiers as session-scoped staging. Narrative accumulates there during active research and drains into the tier docs during sync. It loads alongside the core while research is active; after sync it should be empty or pointer-only.
...
## Why more rules stop helping

The intuition that more rules produce better behavior breaks down, but not for the reason first reported. An early reading of the AGENTS.md paper (arxiv 2602.11988) claimed a measured "cliff" past 500 lines. Read firsthand, the paper says no such thing. What it actually found is sharper: LLM-generated context files produce no significant change in success rates (p=.87) while adding roughly 20% cost, and file length has no measurable effect on outcomes at all. Auto-generated context is busywork, not lift.

The real ceiling is a rule budget, not a line count. A model reliably tracks on the order of 200 rules at once, and the system prompt already spends about 50 of them. Past that, and independent of file length, adherence to context-file rules sits around **70%**. Practitioners converged on that number through session analysis. The implication for anything safety-critical: if a rule must hold every time ("never push to main," "always run tests before commit"), a context file is the wrong place for it. That's a hook. The context file is advisory; hooks are deterministic.
