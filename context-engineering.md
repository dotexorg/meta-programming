...
The goal is a context that's *minimal but sufficient* — not a dump of everything that might be useful. Auto-generated context actually **hurts agent performance by 3%** (ETH Zurich). 🟡 The insight is counterintuitive but consistent: more context isn't free. Every token you add is a token the model has to process and attend to. The principle is minimal effective context — only include information the agent cannot discover on its own.
...
The pattern: a scout agent reads the full repository and writes a structured report. All subsequent worker agents receive the scout report, not the raw codebase. Orchestrators never read files directly. 🟡
...
Prompt caching works on **prefix matching** (Anthropic API): the system prompt, tools, and initial messages must be identical to get a cache hit. A cache hit cuts TTFT (time to first token) by 13–31% and cost by 45–80% (PwC, *Don't Break the Cache*). 🟡
