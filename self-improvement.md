...
Form factor at extraction time matters as much as cadence, and two complementary primitives from one open-source orchestrator (lopi, May 2026) make the cheap-extraction shape concrete. The first is a single-imperative form: each lesson is one line starting with `must`, `do not`, `always`, or `never`, capped at 200 characters, behavior-specific rather than principled — a bounded scope at write time that limits how far the extracting model can drift into general advice unrelated to the actual failure. The second is a score gate at write time: a run scoring below `LESSON_QUALITY_GATE = 0.6` writes nothing, because a sub-threshold run isn't informative enough to generalise from and persisting it would degrade future retrieval. Form-bounding addresses the model-drift failure mode at the moment of capture; quality-gating addresses the noise-accumulation failure mode at the same step. Each is useful alone, but they target different failure modes and pair naturally because both apply at the same write boundary. 🟡
