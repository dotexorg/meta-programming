...
| Date | What landed |
|------|-------------|
| [2026-06-21–22](./changelog/2026-06-21-acceptance-gate-oracle-coverage-failure-taxonomy-cognitive-companion.md) | The headline result: a deterministic acceptance-gate only earns its keep where tests already cover the bug. For the classes they miss (security, crypto, timing) it runs green and catches nothing, so independent review stays the real guard (Exp 10). The "stuck agent" model also grew into a two-level failure taxonomy, execution and cognition. And separate builder/reviewer crossed from best practice to a default that ships in real tools (Cursor, Devin). |
